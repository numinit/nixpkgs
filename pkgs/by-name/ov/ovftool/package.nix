...
let
  ...
  fetchFromBroadcom =
    ...
    fetchurl {
      ...
      postFetch = ''
        # Remove the specified curlOptsList from the curl opts.
        extraOpts=()
        eval "extraOpts+=($curlOptsList)"
        extraOpts+=($curlOpts)
        newOpts=()

        for opt in "''${curl[@]}"; do
          found=0
          for extraOpt in "''${extraOpts[@]}"; do
            if [ "$opt" == "$extraOpt" ]; then
              # Don't look for it again
              extraOpts=("''${extraOpts[@]/$extraOpt}")
              found=1
              break
            fi
          done
        ...
        url="$(jq -r 'if (.success == true) then .data.downloadUrl else error(. | tostring) end' < "$downloadedFile" || exit $?)"
        downloadedFile="$out"
        curl=("''${newOpts[@]}")
        tryDownload "$url"
      '';
